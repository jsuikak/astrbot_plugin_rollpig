...
当你希望让 Codex 按统一规则自动设计并落库新小猪时，推荐使用以下流程：

1. 在 Codex 会话里**先手动上传原图参考**（建议上传 `resource/image/pig.png`）。 (直接让codex生成也可能可行)
2. 用自然语言给出设计需求（例如“以吉他为主题创建一只新的趣味小猪”），Codex 将按仓库内 `SKILL.md` 的规则执行设计与落库。

说明：

- 若未上传原图参考，Codex 会先提示你上传，再继续生图。  
- 生图后会按技能规则做尺寸统一（`240x240`）、图片落盘与 `pig.json` 字段更新。  
